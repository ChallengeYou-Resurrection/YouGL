...
std::vector<std::shared_ptr<CYGeneric>> GeoOctree::getCollisionMeshAtPoint(const glm::vec3& point)
{
	if (!subdivided)
		return m_objects;
...
std::optional<std::shared_ptr<CYGeneric>> GeoOctree::getObjectClosestToRay(const MouseRay::Ray & mRay, u8 maxFloor)
{
